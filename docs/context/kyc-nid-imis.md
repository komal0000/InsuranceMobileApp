...
- The previous HIB data sync page stores the matched household/member setup locally, then sends OTP through `/api/affiliation/otp/send`. On the OTP/password step, "Use a different phone number" calls `/api/affiliation/otp/reset`, clears the phone, OTP, and password fields, and returns to the phone step while the backend keeps the original household/member token and resend cooldown.
